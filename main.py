...
if len(sys.argv) != 8:
    print("Usage: python main.py <TICKER> <TIME_FRAME> <UPDATE_MIN> <REQ_PERIOD> <PROFITABLE> <SHORT_MA> <LONG_MA>")
    exit(1)
...
interval = 60.0 * int(sys.argv[4])
profitable = sys.argv[5]
short_ma_period = int(sys.argv[6])
long_ma_period = int(sys.argv[7])
red = 0xFF0000
green = 0x008000

print(ticker, time_frame, req_period, interval)
    ...
    while not client.is_closed():
        try:
            ...
            if last_bar['Crossover']:
                ...
                # em1.add_field(name="Short MA", value=str(short_ma_period))
                # em1.add_field(name="Long MA", value=str(long_ma_period))
